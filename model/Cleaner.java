...
import controller.CleanerFactory;
import controller.OilFactory;
...
//publikus metódusok kezdete
	...
	public void move(){
		if(this.state!=RobotState.Eliminated){						//csak akkor lép, ha "él"
			...
			this.dir	=	this.map.getRouteToTrap(this);
			if(this.state==RobotState.Unturnable) {
				this.state=RobotState.Normal;
				this.dir	=	new Point(-this.dir.y, this.dir.x);
			}
			if(target!=null){
				if(target.getPosition().equals(this.position)){
					...
					this.position.translate(this.dir.x, this.dir.y);
	 ...
	 */
	public void destroy(){
		this.currentField.addTrap(OilFactory.create(this.position));	//olajfoltot lak re a mezõre amin van
		this.state=RobotState.Eliminated;
